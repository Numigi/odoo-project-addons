...
class TestTaskTemplateAddWizard(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.template_b = cls.env["project.task"].create(
            {"name": "Template Task B", "is_template": True}
        )

        cls.task_a = cls.env["project.task"].create(
            {"name": "Task A", "is_template": False}
        )
        cls.task_b = cls.env["project.task"].create(
            {"name": "Task B", "is_template": False}
        )
    ...
    def test_if_is_template_in_context__templates_not_hidden_from_search(self):
        results = self.env["project.task"].search(
            [("is_template", "=", True), ("name", "ilike", "Task A")]
        )
    ...
    def test_invisible_field_emptied_for_templates(self):
        self.template_a.date_deadline = datetime.now().date()
        self.template_a.refresh()
        assert not self.template_a.date_deadline
