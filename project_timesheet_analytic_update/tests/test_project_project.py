...
class TestProjectTimesheetAnalyticUpdate(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.analytic_account_1 = cls.env['account.analytic.account'].create({'id': 1, 'name': 1})
        cls.analytic_account_2 = cls.env['account.analytic.account'].create({'id': 2, 'name': 2})

        cls.project_b = cls.env['project.project'].create({'name': 'projectB',
                                                           'analytic_account_id': cls.analytic_account_1.id,
                                                           })

        cls.timesheet_b = cls.env['account.analytic.line'].create({'name': 'TSHT_b',
                                                                   'unit_amount': 0.5,
                                                                   'project_id': cls.project_b.id, })

        cls.timesheet_c = cls.env['account.analytic.line'].create({'name': 'TSHT_c',
                                                                   'unit_amount': 1,
                                                                   'project_id': cls.project_b.id, })
