...
class TestProjectTask(common.SavepointCase):
    ...
    def test_after_update__remaining_hours_is_new_value(self):
        new_value = 9
        self.task.update_remaining_hours(
            new_value, self.user, comment='Testing')
    ...
    def test_updating_remaining_hours_using_wizard(self):
        new_value = 20
        wizard = self.env['project.task.remaining.hours.update'].with_user(self.user).create({
    ...
    def test_task_remaining_hours_become_0_when_enable_set_remaining_hour_to_0(self):
        ...
        assert self.task.remaining_hours == 10
        self.stage_done.set_remaining_hours_to_0 = True
        self.task._compute_remaining_hours()
        assert self.task.remaining_hours == 0
