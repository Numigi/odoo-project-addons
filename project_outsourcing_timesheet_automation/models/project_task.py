...
class ProjectTask(models.Model):
    ...
    def _add_po_chatter_message(self, ts):
        self.ensure_one()
        message = "<ul>"
        message += _("<li>User: {} </li>").format(self.env.ref("base.user_root").name)
        if ts.employee_id:
        ...
        if ts.task_id:
            ...
            message += _("<li> Status: {} </li>").format(ts.task_id.stage_id.name)
        message += _(
            "<li> A time entry has been automatically created "
            "for PO line {} <br> - {} </li>"
        ).format(ts.purchase_order_line_id.order_id.name, ts.name)
    ...
    def write(self, vals):
        ...
        for record in self:
            if (
                ...
                .create_subcontractors_time_entries
            ):
