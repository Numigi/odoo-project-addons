...
class Project(models.Model):
    ...
    remaining_estimated_hours = fields.Float(
        "Remaining Estimated Hours", compute="_compute_remaining_estimated_hours", store=True
    )
    total_remaining_hours = fields.Float(
        "Total Remaining Work", compute="_compute_remaining_hours", store=True
    )

    @api.depends("milestone_ids", "milestone_ids.estimated_hours", "milestone_ids.total_hours")
    def _compute_total_hours(self):
        for project in self:
            project.total_estimated_hours = sum(project.milestone_ids.mapped("estimated_hours"))
    ...
    def _compute_remaining_hours(self):
        for project in self:
            project.total_remaining_hours = sum(project.task_ids.mapped("remaining_hours"))
