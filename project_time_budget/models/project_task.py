...
class ProjectTask(models.Model):
    ...
    def unlink(self):
        deleted_tasks = [(t.project_id, t) for t in
                         self._project_budget_log_required()]
        res = super().unlink()
    ...
    def _notify_template_task_added(self):
        for task in self:
            task._notify_template_task_changed_to_project(ADDED,
                                                          task.project_id)

    def _notify_template_task_modified(self):
        for task in self:
            task._notify_template_task_changed_to_project(MODIFIED,
                                                          task.project_id)
    ...
    def _notify_template_task_archived(self):
        for task in self:
            task._notify_template_task_changed_to_project(ARCHIVED,
                                                          task.project_id)

    def _notify_template_task_restored(self):
        for task in self:
            task._notify_template_task_changed_to_project(RESTORED,
                                                          task.project_id)
