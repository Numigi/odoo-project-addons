...
class TestProjectMilestoneTimelineDynamic(SavepointCase):
    ...
    def test_update_start_date_end_date_after_adding_dependence(self):
        ...
        self.milestone_abc.write({"child_ids": [(4, self.milestone_zzz.id)]})
        self.milestone_abc._onchange_child_ids()
        assert self.milestone_abc.start_date == fields.Date.from_string("2022-09-16")
        assert self.milestone_abc.target_date == fields.Date.from_string("2022-11-15")
    ...
    def test_update_start_date_end_date_after_after_updating_dependence(self):
        ...
        self.milestone_1.write({"target_date": "2022-09-15"})

        # Milestone 3 mustn't change
        assert self.milestone_3.start_date == fields.Date.from_string("2022-10-01")
        assert self.milestone_3.target_date == fields.Date.from_string("2022-12-30")

        self.milestone_1.write({"target_date": "2022-10-15"})
        self.milestone_3._onchange_child_ids()
        # Date start and Date end of Milestone 3 must change
        assert self.milestone_3.start_date == fields.Date.from_string("2022-10-16")
        assert self.milestone_3.target_date == fields.Date.from_string("2023-01-14")

        self.milestone_2.write(
            {"start_date": "2022-09-01", "target_date": "2022-10-30"}
        )
        self.milestone_3._onchange_child_ids()
        # Date start and Date end of Milestone 3 must change
        assert self.milestone_3.start_date == fields.Date.from_string("2022-10-31")
        assert self.milestone_3.target_date == fields.Date.from_string("2023-01-29")
