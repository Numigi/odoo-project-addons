...
class TestTask(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.project_stage_no_timesheet = cls.env["project.project.stage"].create(
            {"name": "project_stage_no_timesheet", "allow_timesheets": False}
        )
        cls.project_timesheet = cls.env["project.project"].create(
            {"name": "Project Timesheet", "stage_id": cls.project_stage_timesheet.id}
        )

        cls.project_no_timesheet = cls.env["project.project"].create(
            {
                "name": "Project No Timesheet",
                "stage_id": cls.project_stage_no_timesheet.id,
            }
        )

        cls.task_no_timesheet = cls.env["project.task"].create(
            {"name": "Task 01", "project_id": cls.project_no_timesheet.id}
        )
        cls.task_timesheet = cls.env["project.task"].create(
            {"name": "Task 02", "project_id": cls.project_timesheet.id}
        )
